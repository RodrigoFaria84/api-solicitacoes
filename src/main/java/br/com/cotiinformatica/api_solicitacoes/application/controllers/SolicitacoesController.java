...
public class SolicitacoesController {
    ...
    @PostMapping
    public ResponseEntity<SolicitacaoResponse> post(@Valid @RequestBody SolicitacaoRequest request) throws Exception {
        var response = solicitacaoService.criar(request);
        return ResponseEntity.status(201).body(response);
    }

    @PutMapping("/{id}")
    public ResponseEntity<SolicitacaoResponse> put(@PathVariable UUID id, @Valid @RequestBody SolicitacaoRequest request) throws Exception {
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<SolicitacaoResponse> delete(@PathVariable UUID id) throws Exception {
        var response = solicitacaoService.excluir(id);
        return ResponseEntity.ok().body(response);
    }

    @GetMapping
    public ResponseEntity<Page<SolicitacaoResponse>> get(
